...
CREATE TABLE IF NOT EXISTS insurance_policies (
  id                UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id           UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  type              VARCHAR(30) NOT NULL CHECK (type IN ('term','mediclaim','lic','motor','vehicle','health','life','ulip','other')),
  policy_number     VARCHAR(100),
  provider          VARCHAR(100) NOT NULL,
  plan_name         VARCHAR(255) NOT NULL,
  sum_assured       NUMERIC(15,2),
  premium_amount    NUMERIC(15,2),
  premium_frequency VARCHAR(20) DEFAULT 'annual' CHECK (premium_frequency IN ('monthly','quarterly','half_yearly','annual','yearly')),
  start_date        DATE,
  end_date          DATE,
  maturity_date     DATE,
...
CREATE TABLE IF NOT EXISTS rentals (
  ...
  property_type   VARCHAR(30) CHECK (property_type IN ('apartment','house','commercial','plot','residential','pg','land','custom','other')),
  address         TEXT,
...
CREATE TABLE IF NOT EXISTS notes (
  ...
  title         VARCHAR(255) NOT NULL,
  content       TEXT,
  note_type     VARCHAR(20) DEFAULT 'text' CHECK (note_type IN ('text','voice','scan','checklist','general')),
  tags          TEXT[],
