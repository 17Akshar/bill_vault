...
LOCAL_UPLOAD_DIR = BASE_DIR / "uploads_data"
...
def _try_firebase_upload(
    ...
    try:
        ...
        logger.warning(
            f"Firebase Storage unavailable, using local fallback: {e}"
        )
...
def _save_local(blob_path: str, raw: bytes) -> str:
    ...
    and return a backend-served URL path.
    """
    ...
    # Return a relative API path;
    # frontend prefixes backend URL
...
async def upload_attachment(
    ...
    Returns URL to store in attachment_url.
    """
...
async def serve_local_file(full_path: str):
    """
    Serve locally stored attachment.
    """
    ...
    try:
        ...
        if (
            LOCAL_UPLOAD_DIR.resolve() not in safe.parents
            and safe != LOCAL_UPLOAD_DIR.resolve()
        ):
            raise HTTPException(
                status_code=400,
                detail="Invalid path",
...
async def get_user_labels(request: Request):
    """
    Distinct labels previously used by user.
    """
