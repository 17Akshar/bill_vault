...
import { DEMO_REMINDERS } from './dummyData';
...
type ReminderType = 'salary' | 'freelance' | 'rental' | 'dividend' | 'custom';
type RepeatOption = 'one_time' | 'daily' | 'weekly' | 'monthly' | 'yearly';

const REMINDER_TYPES = [
  { key: 'salary',    label: 'Salary',    icon: 'briefcase-outline',   color: GREEN  },
  { key: 'freelance', label: 'Freelance', icon: 'laptop-outline',      color: PURPLE },
  { key: 'rental',    label: 'Rental',    icon: 'home-outline',        color: ORANGE },
  { key: 'dividend',  label: 'Dividend',  icon: 'bar-chart-outline',   color: '#26C6DA' },
  { key: 'custom',    label: 'Custom',    icon: 'notifications-outline', color: '#FF9100' },
];
...
const STATUS_COLOR: Record<string, string> = {
  upcoming: GREEN, missed: RED, done: PURPLE,
};

type Reminder = {
  id: string; title: string; type: string; amount: number;
  dueDate: string; repeat: string; note?: string; status: string;
};

export default function IncomeReminder() {
  ...
  const [reminders, setReminders] = useState<Reminder[]>(DEMO_REMINDERS as Reminder[]);
  const [showAdd, setShowAdd] = useState(false);

  const [title,     setTitle]     = useState('');
  const [amount,    setAmount]    = useState('');
  const [type,      setType]      = useState<ReminderType>('salary');
  ...
  const handleSave = () => {
    ...
    setReminders(prev => [{
      id: `ir${Date.now()}`, title: title.trim(),
      amount: Number(amount) || 0, type,
      dueDate: format(dueDate, 'yyyy-MM-dd'), repeat,
      note: note.trim() || undefined, status: 'upcoming',
    }, ...prev]);
    setTitle(''); setAmount(''); setNote('');
    setShowAdd(false);
    Alert.alert('Success', 'Income reminder set');
  };
  ...
  const typeColor = (t: string) => REMINDER_TYPES.find(r => r.key === t)?.color || GREEN;
  const typeIcon  = (t: string) => REMINDER_TYPES.find(r => r.key === t)?.icon  || 'notifications-outline';
  ...
  const ReminderCard = ({ item }: { item: Reminder }) => (
    <View style={[styles.remCard, { backgroundColor: CARD }]}>
      ...
      <View style={styles.remBody}>
        ...
        {item.amount > 0 && <Text style={[styles.remAmt, { color: GREEN }]}>{formatINR(item.amount)}</Text>}
        <View style={styles.remMeta}>
          <Ionicons name="calendar-outline" size={12} color={colors.textSecondary} />
          <Text style={[styles.remMetaText, { color: colors.textSecondary }]}>{item.dueDate}</Text>
          <Ionicons name="repeat-outline" size={12} color={colors.textSecondary} style={{ marginLeft: 8 }} />
          <Text style={[styles.remMetaText, { color: colors.textSecondary }]}>
            {REPEAT_OPTIONS.find(r => r.key === item.repeat)?.label || item.repeat}
          </Text>
        </View>
        {item.note && <Text style={[styles.remNote, { color: colors.textSecondary }]}>{item.note}</Text>}
      </View>
      <View style={styles.remActions}>
        {item.status !== 'done' && (
          <TouchableOpacity style={[styles.remBtn, { backgroundColor: `${GREEN}22` }]} onPress={() => markDone(item.id)}>
            <Ionicons name="checkmark" size={15} color={GREEN} />
          </TouchableOpacity>
        )}
        <TouchableOpacity style={[styles.remBtn, { backgroundColor: `${RED}22` }]} onPress={() => deleteReminder(item.id)}>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: BG }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Income Reminders</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsRow}>
          {[{ label: 'Upcoming', count: upcoming.length, color: GREEN },
            { label: 'Missed',   count: missed.length,   color: RED    },
            { label: 'Done',     count: done.length,     color: PURPLE }].map(s => (
        ...
        {done.length > 0 && <>
          <Text style={[styles.sectionTitle, { color: PURPLE }]}>Completed</Text>
      ...
      <TouchableOpacity style={[styles.fab, { backgroundColor: GREEN }]} onPress={() => setShowAdd(true)}>
      ...
      <Modal visible={showAdd} animationType="slide" transparent>
        ...
        <View style={[styles.sheet, { backgroundColor: isDark ? '#1A1A2E' : '#FFF' }]}>
          ...
          <Text style={[styles.sheetTitle, { color: colors.text }]}>New Income Reminder</Text>

          <TextInput style={[styles.input, { borderColor: colors.border, color: colors.text }]} value={title} onChangeText={setTitle} placeholder="Reminder title" placeholderTextColor={colors.textSecondary} />
          <TextInput style={[styles.input, { borderColor: colors.border, color: colors.text }]} value={amount} onChangeText={setAmount} placeholder="Expected amount (optional)" placeholderTextColor={colors.textSecondary} keyboardType="decimal-pad" />

          <Text style={[styles.sheetLabel, { color: colors.textSecondary }]}>Reminder Type</Text>
          <View style={styles.typeRow}>
            {REMINDER_TYPES.map(t => (
              <TouchableOpacity key={t.key} style={[styles.typeChip, type === t.key && { backgroundColor: t.color }]} onPress={() => setType(t.key as ReminderType)}>
                <Ionicons name={t.icon as any} size={13} color={type === t.key ? '#000' : colors.textSecondary} />
                <Text style={[styles.typeText, { color: type === t.key ? '#000' : colors.textSecondary }]}>{t.label}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <Text style={[styles.sheetLabel, { color: colors.textSecondary }]}>Expected Date</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.repeatRow}>
            {REPEAT_OPTIONS.map(r => (
              <TouchableOpacity key={r.key} style={[styles.repeatChip, repeat === r.key && { backgroundColor: GREEN }]} onPress={() => setRepeat(r.key as RepeatOption)}>
                <Text style={[styles.repeatText, { color: repeat === r.key ? '#000' : colors.textSecondary }]}>{r.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>

          <TextInput style={[styles.input, { borderColor: colors.border, color: colors.text }]} value={note} onChangeText={setNote} placeholder="Note (optional)" placeholderTextColor={colors.textSecondary} />

          <View style={styles.switchRow}>
            <Text style={[styles.switchLabel, { color: colors.text }]}>Auto notification alert</Text>
            <Switch value={autoAlert} onValueChange={setAutoAlert} trackColor={{ false: colors.border, true: GREEN }} thumbColor="#000" />
          </View>

          <TouchableOpacity style={[styles.sheetSaveBtn, { backgroundColor: GREEN }]} onPress={handleSave}>
            <Text style={styles.sheetSaveBtnText}>Set Reminder</Text>
          </TouchableOpacity>

          {showDate && (
            <DateTimePicker value={dueDate} mode="date" display="default"
...
const styles = StyleSheet.create({
  ...
  remCard:          { flexDirection: 'row', borderRadius: 14, padding: 14, marginBottom: 10, gap: 12, alignItems: 'flex-start' },
  ...
  remMetaText:      { fontSize: 11 },
  remNote:          { fontSize: 12, fontStyle: 'italic' },
  remActions:       { gap: 6 },
  remBtn:           { width: 30, height: 30, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  ...
  sheetSaveBtnText: { color: '#000', fontSize: 15, fontWeight: '800' },
});
