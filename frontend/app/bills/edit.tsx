...
export default function EditBillScreen() {
  ...
  const loadBill = async () => {
    try {
      ...
      setNotes(bill.notes || '');
      setReceiptImage(bill.receipt_image);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', error.response?.data?.detail || 'Failed to update bill');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Bill</Text>
